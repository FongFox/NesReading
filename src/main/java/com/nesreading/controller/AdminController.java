...
public class AdminController {
    ...
    public String handleCreateBook(Model model, @ModelAttribute("newBook") Book book, @RequestParam("authorIds") List<Long> authorIds, @RequestParam("bookCategoryId") long bookCategoryId) {
    	List<Author> authors = new ArrayList<>(authorService.handleFetchAllAuthorsById(authorIds));
        book.setAuthors(authors);
        
        bookService.handleCreateBook(book, authorIds, bookCategoryId);
    ...
    public String handleUpdateBook(@ModelAttribute("book") Book book, @RequestParam List<Long> authorIds, @RequestParam long bookCategoryId) {
    	bookService.handleUpdateBook(book, authorIds, bookCategoryId);
    	
        return "redirect:/admin/books";
    }

    @GetMapping("/books/delete/{id}")
    public String getBookDeletePage(Model model, @PathVariable long id) {
    	Book tempBook = bookService.handleFetchBookById(id).orElse(null);
    ...
    public String handleDeleteBook(Model model, @ModelAttribute("tempBook") Book tempBook) {
    	bookService.handleDeleteBook(tempBook.getId());
